...
# 构建两个隐藏层，分别为256个神经元和64个神经元
H1_NN = 256
H2_NN = 64

# 输入层：第一个隐藏层参数和偏置项
W1 = tf.Variable(tf.truncated_normal([784, H1_NN], stddev=0.1))
# tf.truncated_normal生成截断型（如果标准差大于后面给定的值，则会放弃该数据，重新生成）、符合正态分布的数据，这样可以使得生成的数据相对比较集中，建议多层神经网络一般情况下使用截断型正态分布数据的生成
b1 = tf.Variable(tf.zeros([H1_NN]))
# 第一个隐藏层到第二个隐藏层的参数和偏置项
